  ...
  <v-row class="d-flex flex-column align-center">
    <v-col class="d-flex flex-column align-center ma-5">
      <img
        ...
        class="mb-5"
      >
      <h1>Histoire de la légitime defense</h1>
    </v-col>
    <v-divider />
    <ul>
      <li v-for="article in sommaire" :key="article.id">
        <v-card>
          <div class="print pa-5">
            ...
            <v-btn
              :to="`/legitime-defense/histoire/articles/${article.slug}`"
            >
              Lire
            </v-btn>
          </div>
...
export default {
  async asyncData ({ $content, error }) {
    const sommaire = await $content('defense/histoire').only(['title', 'description', 'write', 'slug'])
      .fetch()
      .catch((err) => {
